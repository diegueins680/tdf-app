...
test('formal runner and both hosted triggers retain the fail-closed integrity gate', () => {
  ...
  for (const name of ['scripts/verify-event-operations-pluscal.mjs', 'scripts/__tests__/event-operations-pluscal.test.mjs']) {
    assert.equal(workflow.split(`"${name}"`).length - 1, 2);
  }
