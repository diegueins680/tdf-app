...
// Invariant: if an amd64 Linux tarball exists, select it before any weaker fallback.
export function selectKoyebDownloadUrl(candidates) {
...
// Invariant: only executable regular files named exactly "koyeb" satisfy the postcondition.
export async function findExtractedKoyebBinary(rootDir) {
  ...
  while (pendingDirs.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (!entry.isFile() || entry.name !== 'koyeb') {
        continue;
      }
