...
# Legacy corruption must abort the whole installation, without deleting edges
# or leaving newly created foundation tables behind. These rows are test-owned.
assert_legacy_graph_rejected() {
  psql_exec -c "$1" >/dev/null
  legacy_edges=$(psql_exec -qAt -c 'SELECT array_agg(ROW(id,activity_id,depends_on_activity_id)::text ORDER BY id) FROM event_logistics_dependency;')
  if graph_result=$(docker exec -i "$test_container" psql -X -v ON_ERROR_STOP=1 -v VERBOSITY=verbose -U postgres -d "$test_database" < "$up_migration" 2>&1); then
    echo 'expected legacy dependency graph rejection' >&2
    exit 1
  fi
  case "$graph_result" in *23514*"$2"*) ;; *) echo 'unexpected legacy graph failure' >&2; exit 1;; esac
  test "$(psql_exec -qAt -c "SELECT to_regclass('event_operation_task_policy') IS NULL;")" = t
  test "$(psql_exec -qAt -c 'SELECT array_agg(ROW(id,activity_id,depends_on_activity_id)::text ORDER BY id) FROM event_logistics_dependency;')" = "$legacy_edges"
  psql_exec -c 'DELETE FROM event_logistics_dependency;' >/dev/null
}
assert_legacy_graph_rejected 'INSERT INTO event_logistics_dependency(activity_id,depends_on_activity_id) VALUES (100,101),(101,103),(103,100);' 'graph contains a cycle'
assert_legacy_graph_rejected 'INSERT INTO event_logistics_dependency(activity_id,depends_on_activity_id) VALUES (100,100);' 'graph contains a cycle'
assert_legacy_graph_rejected 'INSERT INTO event_logistics_dependency(activity_id,depends_on_activity_id) VALUES (100,102);' 'must remain within one event'
