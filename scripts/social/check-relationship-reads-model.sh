...
for config in RelationshipReads RelationshipReadsPolicy RelationshipReadsCounts RelationshipReadsDeleted; do
  set +e
  "$TDF_SOCIAL_JAVA" -cp "$TLA_JAR" tlc2.TLC -workers 1 -deadlock \
    ...
    "$TDF_SOCIAL_ROOT/formal/social/RelationshipReads.tla" > "$TDF_SOCIAL_RESULTS/$config.txt" 2>&1
  result=$?
  set -e
  if [ "$config" = RelationshipReads ]; then
    [ "$result" = 0 ]
    grep 'Model checking completed. No error has been found.' "$TDF_SOCIAL_RESULTS/$config.txt"
  else
    [ "$result" != 0 ]
    property=RowsAuthorized
    [ "$config" != RelationshipReadsCounts ] || property=NoPrivateCounts
