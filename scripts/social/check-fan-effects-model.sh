...
for config in FanEffects FanEffectsLocks FanEffectsPause FanEffectsGoverned FanEffectsRetry; do
  set +e
  "$TDF_SOCIAL_JAVA" -cp "$TLA_JAR" tlc2.TLC -workers 1 -deadlock \
    ...
    "$TDF_SOCIAL_ROOT/formal/social/FanEffects.tla" > "$TDF_SOCIAL_RESULTS/$config.txt" 2>&1
  result=$?
  set -e
  if [ "$config" = FanEffects ]; then
    [ "$result" = 0 ]
    grep 'Model checking completed. No error has been found.' "$TDF_SOCIAL_RESULTS/$config.txt"
  else
    [ "$result" != 0 ]
    property=NoImplicitEffects
    [ "$config" != FanEffectsRetry ] || property=NoDuplicateAlerts
