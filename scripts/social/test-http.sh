...
export TDF_SOCIAL_HTTP_DB="host=127.0.0.1 port=$TDF_SOCIAL_PORT user=postgres password=synthetic-only dbname=social_http"
cd "$TDF_SOCIAL_ROOT"
TDF_SOCIAL_HTTP_BUILD=$(mktemp -d)
trap 'docker rm -f "$TDF_SOCIAL_CONTAINER" >/dev/null 2>&1 || true' EXIT
...
"$TDF_SOCIAL_HTTP_BUILD/social-http-spec"
