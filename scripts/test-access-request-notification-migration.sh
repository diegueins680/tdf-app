...
type_compatibility_repair="$repo_root/tdf-hq/sql/2026-09-08_notification_notif_type_text_compatibility.sql"
...
psql_exec -c "DELETE FROM notification WHERE notif_type LIKE 'access_request_%';" >/dev/null
apply_file "$down_migration"
psql_exec -c 'ALTER TABLE notification ALTER COLUMN notif_type TYPE varchar
  USING notif_type::varchar;' >/dev/null
apply_file "$up_migration"
insert_notification access_request_submitted
...
psql_exec -c "INSERT INTO notification (recipient_party_id, notif_type, title, body)
  VALUES (1, NULL, 'preserve', 'preserve');" >/dev/null
if apply_file "$type_compatibility_repair" 2>/dev/null; then
  echo "Notification type compatibility accepted a NULL varchar value" >&2
  exit 1
fi
type_repair_rejected_state=$(psql_exec -Atc "SELECT data_type || ':' || is_nullable || ':' ||
    (SELECT count(*)::text FROM notification WHERE notif_type IS NULL)
  FROM information_schema.columns
  WHERE table_schema='public' AND table_name='notification' AND column_name='notif_type';")
if [ "$type_repair_rejected_state" != "character varying:YES:1" ]; then
  echo "Notification type compatibility did not preserve rejected NULL varchar data" >&2
  exit 1
fi

psql_exec -c 'DELETE FROM notification WHERE notif_type IS NULL;' >/dev/null
apply_file "$type_compatibility_repair"
apply_file "$type_compatibility_repair"
type_repair_state=$(psql_exec -Atc "SELECT data_type || ':' || is_nullable
  FROM information_schema.columns
  WHERE table_schema='public' AND table_name='notification' AND column_name='notif_type';")
if [ "$type_repair_state" != "text:NO" ]; then
  echo "Notification type compatibility did not establish non-null text" >&2
  exit 1
fi
if [ "$(psql_exec -Atc "SELECT count(*) FROM notification
  WHERE notif_type IN ('test_sink_captured', 'test_sink_failed');")" != "2" ]; then
  echo "Notification type compatibility did not preserve legacy producer values" >&2
  exit 1
fi

psql_exec -c "ALTER TABLE notification ADD CONSTRAINT notification_notif_type_check
  CHECK (notif_type = 'reaction_received') NOT VALID;" >/dev/null
if apply_file "$up_migration" 2>/dev/null; then
  echo "Access-request migration accepted a malformed named notification constraint" >&2
  exit 1
fi
if [ "$(psql_exec -Atc "SELECT count(*) FROM pg_constraint
  WHERE conrelid='public.notification'::regclass
    AND conname='notification_notif_type_check' AND NOT convalidated;")" != "1" ]; then
  echo "Access-request migration did not preserve the rejected malformed constraint" >&2
  exit 1
fi

echo "Access-request notification migration passed forward, idempotency,"
echo "constraint, rollback-safety, forward-repair, and legacy-varchar compatibility checks."
