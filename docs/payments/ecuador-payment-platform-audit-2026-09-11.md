...
**Market verification date:** 2026-09-11 (America/Guayaquil)
...
| PlaceToPay | Missing operational credentials/UX; shared executor implemented disabled | Create/status endpoint, exact account method restriction, signed notification, authenticated query, fixed hosts, redirect allowlist, amount/currency/reference binding, durable ambiguous-operation handling, and mocked contract tests exist. No client return flow, TDF credentials, or sandbox evidence. |
| PayPhone | Missing operational credentials/UX; shared executor implemented disabled | API Sale create/status, authenticated reconciliation, official external-notification aliases, exact binding, fixed host, and mocked contract tests exist. Browser/external notification is never payment evidence. No client polling/app-switch flow, TDF credentials, or controlled-test evidence. |
...
- PlaceToPay and PayPhone shared execution and notification/query reconciliation are implemented, but legacy product selectors deliberately omit them until web/mobile return, app-switch, polling, restore, and accessible error UX is wired and credentialed tests pass.
...
| PayPhone wallet/QR | Distinct Ecuador wallet and merchant QR | Yes | PayPhone | No public cryptographic notification scheme was found; authenticated query is mandatory. | Backend create/query/notification implemented; client UX and provider verification blocked. |
...
- PlaceToPay: requires login/secret, HTTPS return/notification URLs, at least one exact site payment-method mapping, ready account/contract/credential rows, exact verified capabilities, and notification/worker flags. The backend executor exists; customer exposure remains disabled until sandbox and client-return evidence exist.
- PayPhone: requires token/store/response URL, ready account/contract/credential rows, exact verified wallet capabilities, and notification/worker flags. The backend executor and `NotificacionPago` compatibility route exist; customer exposure remains disabled until controlled-test and client polling/app-switch evidence exist.
...
2. Configure the implemented shared executor with the exact site payment-method IDs, register return/notification URLs, and activate exact sandbox rows only. Wire the dependent web/mobile return and restore UX.
...
3. Register the implemented `/NotificacionPago` notification compatibility path, wire the dependent app-switch/polling UX, and run API Sale/query/cancel/reversal/timeout/duplicate/notification tests. Do not treat collaborator transfers as split settlement.
...
Local delivery branches are `codex/payment-platform-20260911` in `tdf-app` and `codex/provider-neutral-ticket-checkout-20260911` in `TDF-mobile`. The mobile branch head is `c1ae3e34b53f8e709d361677d20b9778bdf87d61`; GitHub verified draft [TDF-mobile PR #78](https://github.com/diegueins680/TDF-mobile/pull/78), based on the parent-referenced #64 lineage. It explicitly supersedes stale generated-only PR #76. GitHub verified draft [tdf-app PR #331](https://github.com/diegueins680/tdf-app/pull/331) for the parent completion branch. The complete local regression covered parent commit `2eac48e2b4b0588ad162fe7fd44808b799261567`; focused corrections culminated in the all-green hosted code/evidence commit `ac018a3f1afed8e59c8f7d3c4188ecc01e709e72`. No merge or production deployment is part of this work.
