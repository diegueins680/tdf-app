...
- A closed provider-adapter boundary now represents create, query, cancel, capture, void, refund and same-day reversal operations without accepting PAN, CVV or magnetic-stripe data. Request bodies, buyer inputs and nonce context intentionally have no `Show` instance; log output is limited to a redacted request summary.
- The shared transport accepts only allowlisted provider HTTPS hosts, disables redirects to prevent credential forwarding, applies a 15-second timeout and 1 MiB response limit, and converts network, status and JSON failures to payload-free public errors.
...
| 2026-09-10 local / `27138b013` | `stack test --test-arguments=--match=provider --fast` | local adapter/unit/property | 69 examples, 0 failures. Includes exact binding, fixed-host redirect and cross-provider transport rejection, PlaceToPay signature verification, PayPhone untrusted callback/query, same-day reversal cutoff, capability gating and safe fallback. |
