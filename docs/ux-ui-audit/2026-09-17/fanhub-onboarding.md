...
### Native cross-device contract counterexample (2026-09-18)

UX-260917-037 was reproduced on an actual iOS simulator and isolated PostgreSQL/HTTP, not a mocked API. The previous native onboarding used `/social-events/artists/:id/follow`; FanHub uses `/fans/me/follows/:partyId`. Only the latter creates the authoritative engagement evidence. The first request persisted a social-event follow but `/session/onboarding/complete` returned `newlyCompleted:false` with no completion.

The correction uses the existing FanHub catalog and follow contract, reads server follows under the current Party query key, checks owner authority before dispatch and receipt, and retains the completion handshake. The existing FanHubOnboarding model covers current-session and persisted-receipt transitions; it does not prove endpoint selection. The actual HTTP/native/web test supplies that missing conformance evidence. Old-path negative and corrected-path positive receipts are in `evidence/native-canonical-follow-{negative,positive}.json`. Source and final artifact qualification remain separate; this is a simulator production JS bundle using the existing native QA shell, not signed store or physical Google OAuth verification.
