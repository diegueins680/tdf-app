...
- Build web de producción: PASS; presupuesto inicial de JavaScript PASS (416.063 bytes gzip).
...
- Jest web focalizado `tdf-hq-ui/src/api/merch.test.ts` + `tdf-hq-ui/src/utils/merch.test.ts`: PASS, 10/10. La nueva suite comprueba exportación operativa sin PII, columnas financieras condicionadas y neutralización de fórmulas CSV.
