...
- Jest web focalizado `MerchAdminPage.test.tsx` + API/cliente + exportación CSV: PASS, 29/29. Comprueba consola de liquidación en espera/aprobada, Axe sin impactos serios/críticos, mensajes que niegan payout, multipart idempotente, evidencia sin solicitud de transferencia, exportación sin PII, columnas financieras condicionadas y neutralización de fórmulas CSV.
...
- Ejecución financiera completa de refund/dispute y ejercicio humano de la liquidación contra extracto real autorizado en staging. El flujo técnico de evidencia final ya está implementado y verificado solo con datos/archivos sintéticos; registrar evidencia no mueve dinero.
