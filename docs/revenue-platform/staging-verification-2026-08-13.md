...
| Marketplace customer operations migration | `./scripts/test-marketplace-operations-migration.sh` | Pass on an isolated local PostgreSQL 16 cluster: rerun/clean rollback, customer-request transition guards, quote-only extensions, dispute/payment separation, independent deposit review, balanced liability settlement, credit note, no fabricated provider refund, and evidence-preserving rollback. The same script is registered in CI for PostgreSQL 17; that CI result remains pending until the branch is pushed. |
...
| Marketplace customer-operation backend build | `stack test --fast --no-run-tests`; clean `stack test tdf-hq:test:tdf-hq-test --fast --test-arguments "--match marketplace customer request and deposit settlement invariants"` | The first pass compiled and linked the executable and test component after the customer-operation implementation. A later clean GHC 9.10.3 pass compiled `CheckoutStore`, `MarketplaceOperations`, `MarketplaceRentals` and the new tests, then was stopped at module 62/170 before disk exhaustion from an unrelated concurrent worktree. The final route-name-only correction still requires the clean CI build; no focused backend test pass is claimed for this slice yet. |
...
reconciliation work rather than fabricated payment. After synchronizing current `main`, final local
