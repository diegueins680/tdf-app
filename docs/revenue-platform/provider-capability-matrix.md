...
|---|---|---|---|---|---|---|
| Datafast | Canonical service marketplace sale/rental booking course and public ticket checkout/resource creation plus server status confirmation | Immutable checkout/resource path amount USD currency environment merchant entity internal order and merchant reference | Explicitly disabled; no merchant refund contract verified including rental-deposit settlement | Not assumed | Not offered by this checkout integration | Marketplace domain flags are enabled; other domain and provider execution webhook and refund flags remain off pending certification and authorization |
| PayPal Checkout | Canonical service marketplace sale/rental booking course and public ticket Orders v2 create/capture plus shared verified webhook inbox service refund and reconciliation | Remote signature verification over the exact raw event; event replay window/deduplication; order/capture/path/custom ID amount currency environment and payee merchant; request-id retry protection | Two-person service refund request/approval is implemented; non-zero marketplace deposit and public ticket refund execution remain gated; all production refund execution is off | Separate Subscriptions capability not enabled | Separate Payouts product not enabled | Marketplace domain flags are enabled; public ticket and production PayPal execution remain off pending credentialed sandbox evidence and authorization |
...
| Bank/cash/POS | Canonical manual attempt, customer evidence submission and protected finance review for services and marketplace sale/rental checkout | Independent authorized staff approval with exact immutable checkout/attempt/amount/currency/environment binding; browser selection and reference submission never mean payment | Manual rental-deposit liability settlement records exact server-derived amounts, evidence, balanced ledger and credit-note reference; it does not claim a provider refund | No | Admin-reviewed settlement only | Marketplace sale/rental domain flags, manual methods, and dual-control deposit settlement are enabled; production deployment remains separate, and expired holds, provider refunds and every external settlement require reviewed evidence |
...
Server-only variable names currently read by the shared service and marketplace sale/rental adapter:
...
Guest public ticket status/provider actions additionally require
`COMMERCE_LOOKUP_TOKEN_SECRET`, a server-only secret-manager value of at least 32 UTF-8 bytes. It
keys the pseudorandom order capability so idempotent checkout recovery does not require storing a
plaintext token and knowledge of the public/client idempotency key is insufficient to derive one.
Rotate it through a planned capability migration; changing it immediately invalidates outstanding
...
For the implemented service and marketplace sale/rental slices, provider verification mismatch creates an open reconciliation
...
Rental payment confirmation changes only the reservation state; custody and deposit settlement use
separate audited transitions. The refund and webhook runtimes are implemented but remain
capability-gated. Local fixtures are not provider sandbox evidence or production authorization.
