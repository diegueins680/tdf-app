...
| Guessing guest orders | High-entropy or server-keyed pseudorandom lookup capability stored only as a hash with constant-shape not-found responses; public tickets add atomic per-event throttling keyed by an HMAC of normalized buyer email; legacy orders without a runtime token fail closed | Add verified-email recovery and expand edge/device abuse telemetry and throttling to every domain |
| Ticket oversell or duplicate issuance | Event tier and promotion rows are locked with expiring atomic holds; issuance locks the canonical runtime and a partial unique index prevents duplicate issued audits | Run production-like concurrent HTTP provider callback tests and alert on capacity or issuance conflicts |
| Organizer proceeds reported as paid out | Ticket captures credit organizer proceeds to a payable liability while settlement is independently disabled | Implement dual-control settlement evidence reconciliation and legal/accounting approval before any organizer settlement |
