...
The production `fly.toml` routes internal feedback uploads to `/data/assets/.internal-feedback`, beneath the existing persistent `tdf_assets` mount. The public raw-asset handler rejects that reserved subtree, and production release validation fails closed when `TDF_INTERNAL_FEEDBACK_UPLOAD_ROOT` is absent, relative, moved outside the private subtree, normalized outside its declared path, or not beneath a persistent mount. This configuration is release readiness only; it does not authorize or perform a production deployment.
