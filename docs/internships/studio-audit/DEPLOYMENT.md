...
```text
APP_ENV=test
RESET_DB=true                 # one initial start in the dedicated database only
SEED_DB=true                  # one initial start in the dedicated database only
...
TDF_INTERNAL_FEEDBACK_UPLOAD_ROOT=/private/staging-only/audit-evidence
```

Secrets and the runtime-only persona password are installed through the staging secret manager and never committed. After the initial seed, set `RESET_DB=false` and `SEED_DB=false`.
...
The migration registry requires an `introducedBy` commit hash. Because commits are not authorized, the registry entry is intentionally pending. Add it only after the approved commit exists; never invent a hash.
