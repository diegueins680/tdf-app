...
| Internship terminal-state contracts | `stack test --fast --test-arguments=--match=internship`; focused Jest for `InternshipsPage` and `InternTaskDetailPage` | Pass: 40 backend examples and 22 web tests; cancelled project/task states normalize and render consistently, while completed audit-plan retries are rejected without restamping approval provenance |
