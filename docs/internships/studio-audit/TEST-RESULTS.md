...
| Backend targeted validation | Targeted Hspec execution-status and internal-feedback validation matches | Pass: 5 execution/completion-validation and 6 feedback/CSV-validation examples |
| Internship terminal-state contracts | `stack test --fast --test-arguments=--match=internship`; focused Jest for `InternshipsPage` and `InternTaskDetailPage` | Pass: 41 backend examples and 22 web tests; cancelled project/task states normalize and render consistently, mixed done/cancelled sibling outcomes deterministically complete the project, and completed audit-plan retries are rejected without restamping approval provenance |
...
| Backend full suite | `stack test --fast` in `tdf-hq` | Pass: 2,439 examples, 0 failures |
