...
| API lifecycle E2E | `TDF_AUDIT_E2E_BACKEND_EXE=<compiled-test-binary> TDF_AUDIT_E2E_PASSWORD=<runtime-only> sh scripts/test-studio-internship-audit-api-e2e.sh` | Pass again on the final review-fix tree in disposable local PostgreSQL: synthetic Manager, assigned Intern, and unrelated Intern; non-pristine task conversion rejection; sibling-safe cancellation; inactive draft visibility and activation; protected audit-task lifecycle and assignment; atomic numbering for two concurrent execution requests; optimistic serialization of concurrent report transitions; post-submission blocker authority; assigned-only initial final summary; automatic immutable retest execution even when a stale execution ID is supplied; category/type mismatch rejection; evidence/history; private report CRUD; duplicate warning; admin triage/info/retest/close; normal administrative approval stamps; rejection of execution and retest mutations after completion; daily/final/calculated completion while a sibling task keeps the project active; export; reporter and immediate team in-app notifications; undispatched test outbox; public feedback; legacy feedback readability; cleanup |
...
| Existing task-detail regression | `npm test --workspace=tdf-hq-ui -- --runInBand src/pages/InternTaskDetailPage.test.tsx` | Pass: 7 tests, including intern/admin enforcement that audit-task project, assignment, status, and calculated progress remain controlled by the audit plan |
