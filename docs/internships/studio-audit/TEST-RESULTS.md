...
| Synthetic staging baseline | `node --test scripts/__tests__/studio-audit-staging-baseline.test.mjs`; render and execute `scripts/baseline-studio-audit-staging-migrations.mjs` against a fresh empty `tdf_studio_audit_staging` schema | Pass: two generator tests; executable SQL normalized current-schema ticket/social-event compatibility, inserted 13 synthetic cutover markers and 61 pre-audit migration checksums, left the audit and later migrations pending, verified counts, and cleaned up the disposable database |
