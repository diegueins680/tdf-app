...
| Commit before receipt validation | API returns failure for a malformed SQL receipt after reassignment already committed. | PR 24 validates the exact event/task/key/parties/role/revision inside the session-fenced transaction; fault injection checks assignment/audit/receipt rollback and deferred COMMIT failure, not just HTTP status. |
...
| Recipient roster disclosure or stale management authority | A task reader gets manager-only recipients, a sibling grant enters the roster, or a pre-wait grant survives expiry. | PR 25 checks current exact-task authority after the metadata wait, hides options from readers, filters candidates through canonical read access, and tests expiry/downgrade/scope/pagination. IDs only, private no-store, no ambient people search or client-selected actor. Context is advisory; the existing command reauthorizes. |
| Duplicate intent or misleading success after network failure | An editor silently rebases, generates a new key after losing a response, or renders a previous session's success. | PR 26 freezes reviewed key/body/revision, permits only explicit identical retries after uncertainty, fences dispatch and receipts to the mounted session, and displays success only after strict client validation. Initial conflict requires a fresh review. Local recovery details are not durable; unload warning and visible manual-history instructions disclose this limitation. |
