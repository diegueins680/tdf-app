...
| EO-024 | Complete RACI, approvers/watchers/escalation | Existing but requiring modification | Canonical RACI/policy and scoped reads now exist. PR 23 adds atomic single-pair reassignment; PR 24 adds its session-fenced API; PR 25 adds current mutation authority, replaceable source pairs and paginated eligible recipient IDs through a strict authorized context API/client. Editor, consent, approvers/watchers/escalation remain incomplete. See [evidence](pr-25-raci-editor-context.md). |
