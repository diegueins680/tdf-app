...
| EO-024 | Complete RACI, approvers/watchers/escalation | Existing but requiring modification | Canonical RACI/policy and scoped reads now exist. PR 23 adds atomic single-pair reassignment; PR 24 exposes an authenticated, session-fenced API and strict typed web client with pre-commit receipt validation. Editor, consent, approvers/watchers/escalation remain incomplete. See [evidence](pr-24-raci-reassignment-api.md). |
