...
| EO-008, EO-023–028, EO-055–056, EO-058 | Preserve legacy identity/history across migration, retry, rollback and reapply | Existing `EventLifecycle`, `ReceiptReplay`, `TaskCommit`, `TaskRaci`, `EventStructure` | Existing event migration chain; no production changes | `test-event-operations-schema-rehearsal.sh`, `event_operations_schema_*.sql`, runner isolation tests, hosted `postgres-complete-schema` | Strict baseline blocked by pre-existing manifest/schema defects; [diagnostic bounds and results](pr-10-schema-rehearsal.md) are not release compatibility |
