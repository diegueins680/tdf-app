              ...
              <Grid key={eventId || `${ev.eventTitle}-${ev.eventStart}`} item xs={12} md={6}>
                <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column', borderRadius: 3 }}>
                  <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, flex: 1 }}>
                    ...
                    {eventCardActionUiState.showRsvpActions && eventId && (
                      <EventRsvpControls
                        eventId={eventId}
                        title={ev.eventTitle}
                        start={ev.eventStart}
                        ...
                        locale={locale}
                        eligible={ev.eventRsvpEligible === true}
                        cancelled={ev.eventWorkflowStateCode === 'cancelled'}
                        publicShareEligible={ev.eventIsPublic === true && ev.eventPublicListable === true}
                        compact
                        origin="event_card"
                      />
                    )}
