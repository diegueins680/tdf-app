...
const stripFirstRunCohortDescriptorPrefix = (title: string) => {
  ...
  const strippedTitle = trimmedTitle
    .replace(
      /^(?:formulario\s+(?:p[uú]blico|del?\s+curso|de\s+inscripci[oó]n|de\s+registro|para\s+el|para)|ficha\s+(?:de\s+inscripci[oó]n|de\s+registro|de\s+matr[ií]cula|del?\s+curso|de\s+curso|para\s+el|para)|p[aá]gina\s+(?:de\s+inscripci[oó]n|de\s+registro|(?:p[uú]blica\s+)?del?\s+curso)|solicitud(?:es)?\s+(?:de\s+inscripci[oó]n|de\s+registro|de\s+matr[ií]cula|del?\s+curso|de\s+curso)|inscripciones?\s+(?:del?\s+curso|de\s+curso)|matr[ií]culas?(?:\s+(?:del?\s+curso|de\s+curso|de\s+inscripci[oó]n|de\s+registro))?|inscripci[oó]n(?:\s+(?:del?\s+curso|de\s+curso|al\s+curso))?|public\s+form|course\s+form|(?:course\s+)?signup(?:\s+(?:form|page))?|(?:public\s+)?course\s+page|(?:course\s+)?(?:registration|enrollment)(?:\s+(?:form|page))?|landing\s+(?:del\s+curso|de\s+curso|de\s+inscripci[oó]n|de\s+registro|para\s+el|para|del|de)|course\s+landing(?:\s+page)?|landing\s+page)(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i,
      '',
    )
    .trim();
...
const stripFirstRunCohortDescriptorSuffix = (title: string) => {
  ...
  const strippedTitle = trimmedTitle
    .replace(
      /\s*(?:[-:/|]\s*)?(?:formulario\s+(?:p[uú]blico|del?\s+curso|de\s+inscripci[oó]n|de\s+registro)|ficha\s+(?:de\s+inscripci[oó]n|de\s+registro|de\s+matr[ií]cula|del?\s+curso|de\s+curso)|p[aá]gina\s+(?:de\s+inscripci[oó]n|de\s+registro|(?:p[uú]blica\s+)?del?\s+curso)|solicitud(?:es)?\s+(?:de\s+inscripci[oó]n|de\s+registro|de\s+matr[ií]cula|del?\s+curso|de\s+curso)|inscripciones?\s+(?:del?\s+curso|de\s+curso)|matr[ií]culas?(?:\s+(?:del?\s+curso|de\s+curso|de\s+inscripci[oó]n|de\s+registro))?|inscripci[oó]n(?:\s+(?:del?\s+curso|de\s+curso|al\s+curso))?|public\s+form|course\s+form|(?:course\s+)?signup(?:\s+(?:form|page))?|(?:public\s+)?course\s+page|(?:course\s+)?(?:registration|enrollment)(?:\s+(?:form|page))?|landing\s+(?:del\s+curso|de\s+curso|de\s+inscripci[oó]n|de\s+registro)|course\s+landing(?:\s+page)?|landing\s+page)\s*$/i,
      '',
    )
    .trim();
