...
const firstRunCoursePlatformPattern = String.raw`(?:kajabi|teachable|thinkific|moodle|hotmart|podia|learn\s*worlds?|click\s*funnels?|clickfunnels|kartra|systeme\s*\.?\s*io|ghl|go\s*high\s*level|gohighlevel|highlevel|lead\s*connector|mighty\s*networks?|skool|circle\s*\.?\s*so|udemy|coursera|skill\s*share|ed\s*x)`;
...
const firstRunSurveyDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:course\s+)?(?:survey|questionnaire|feedback|evaluation)\s+(?:forms?|pages?|portals?|requests?)|(?:post[-\s]?course|end[-\s]?of[-\s]?course|exit|satisfaction|nps)\s+(?:surveys?|questionnaires?|feedback|evaluations?)(?:\s+(?:forms?|pages?|portals?|requests?))?|(?:(?:pre[-\s]?)?registration|enrollment|application|intake)\s+(?:surveys?|questionnaires?|feedback\s+forms?|evaluations?)|(?:formulario|p[aá]gina|solicitud(?:es)?)\s+de\s+(?:encuesta|cuestionario|evaluaci[oó]n|retroalimentaci[oó]n)(?:\s+(?:del?\s+curso|de\s+curso|para\s+el\s+curso))?|(?:encuesta|cuestionario|evaluaci[oó]n|retroalimentaci[oó]n)\s+de\s+(?:pre)?inscripci[oó]n(?:es)?|(?:encuesta|cuestionario)\s+de\s+satisfacci[oó]n|(?:evaluaci[oó]n|retroalimentaci[oó]n)\s+final(?:\s+(?:del?\s+curso|de\s+curso))?|(?:encuesta|cuestionario|evaluaci[oó]n|retroalimentaci[oó]n)\s+(?:del?\s+curso|de\s+curso|para\s+el\s+curso))\s*$/i;
