...
const firstRunFinancialAidDescriptorPrefixPattern =
  /^(?:(?:scholarships?|grants?|financial\s+aid|bursar(?:y|ies))\s+(?:(?:applications?|requests?)(?:\s+(?:forms?|pages?|portals?))?|forms?|pages?|portals?)|(?:forms?|pages?|portals?|applications?|requests?)\s+(?:for\s+)?(?:scholarships?|grants?|financial\s+aid|bursar(?:y|ies))|(?:formulario|p[aá]gina|portal|solicitud(?:es)?)\s+(?:de|para(?:\s+la)?)\s+(?:becas?|ayuda\s+financiera|apoyo\s+econ[oó]mico)|(?:becas?|ayuda\s+financiera|apoyo\s+econ[oó]mico)\s+(?:formularios?|p[aá]ginas?|portales?|solicitud(?:es)?))(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i;

const firstRunFinancialAidDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:scholarships?|grants?|financial\s+aid|bursar(?:y|ies))\s+(?:(?:applications?|requests?)(?:\s+(?:forms?|pages?|portals?))?|forms?|pages?|portals?)|(?:forms?|pages?|portals?|applications?|requests?)\s+(?:for\s+)?(?:scholarships?|grants?|financial\s+aid|bursar(?:y|ies))|(?:formulario|p[aá]gina|portal|solicitud(?:es)?)\s+(?:de|para(?:\s+la)?)\s+(?:becas?|ayuda\s+financiera|apoyo\s+econ[oó]mico)|(?:becas?|ayuda\s+financiera|apoyo\s+econ[oó]mico)\s+(?:formularios?|p[aá]ginas?|portales?|solicitud(?:es)?))\s*$/i;
...
const firstRunEmailMarketingFormDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:convert\s*kit|brevo|sendinblue|flodesk|mailer\s*lite|mailerlite|klaviyo|active\s*campaign|constant\s+contact|keap|infusionsoft)\s+(?:(?:lead|(?:pre[-\s]?)?registration|enrollment|application|sign[-\s]?up|intake|interest|contact|inquiry|enquiry)\s+)?(?:forms?|pages?|portals?)|(?:formularios?|p[aá]ginas?|portales?)\s+(?:de\s+)?(?:convert\s*kit|brevo|sendinblue|flodesk|mailer\s*lite|mailerlite|klaviyo|active\s*campaign|constant\s+contact|keap|infusionsoft))\s*$/i;

const firstRunCoursePlatformPattern = String.raw`(?:kajabi|teachable|thinkific|moodle|hotmart|podia|learn\s*worlds?|click\s*funnels?|clickfunnels|kartra|systeme\s*\.?\s*io|go\s*high\s*level|gohighlevel|mighty\s*networks?|skool|circle\s*\.?\s*so)`;
...
const firstRunCommunityGroupDescriptorPattern = String.raw`(?:(?:(?:whats\s*app|discord|slack|telegram|facebook|fb)\s+(?:community|comunidad|group|grupo|chat))|(?:(?:community|comunidad|group|grupo|chat)\s+(?:de\s+)?(?:whats\s*app|discord|slack|telegram|facebook|fb))|(?:(?:course|class|students?|members?)\s+(?:group\s+chat|chat\s+group|community|group|chat))|(?:(?:group\s+chat|chat\s+group|community|group|chat)\s+(?:for\s+)?(?:the\s+)?(?:course|class|students?|members?))|(?:(?:comunidad|grupo|chat)\s+(?:de(?:l)?\s+curso|para\s+(?:el\s+)?curso|de\s+(?:estudiantes|alumnos|miembros)|para\s+(?:estudiantes|alumnos|miembros))))`;
...
const firstRunLiveSessionDescriptorPrefixPattern =
  /^(?:(?:(?:zoom|google\s+meet|microsoft\s+teams|teams)\s+)?(?:webinar|seminar|meeting|live\s+session|sesi[oó]n\s+en\s+vivo)\s+(?:registration|enrollment|sign[-\s]?up)(?:\s+(?:form|page|portal|link|url))?|(?:zoom|google\s+meet|microsoft\s+teams|teams)\s+(?:registration|enrollment|sign[-\s]?up)(?:\s+(?:form|page|portal|link|url))?|(?:formulario|p[aá]gina|portal|enlace|link|url)\s+de\s+(?:registro|inscripci[oó]n)\s+(?:de|para)\s+(?:webinar|seminario|seminar|meeting|sesi[oó]n\s+en\s+vivo))(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i;

const firstRunLiveSessionDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:(?:zoom|google\s+meet|microsoft\s+teams|teams)\s+)?(?:webinar|seminar|meeting|live\s+session|sesi[oó]n\s+en\s+vivo)\s+(?:registration|enrollment|sign[-\s]?up)(?:\s+(?:form|page|portal|link|url))?|(?:zoom|google\s+meet|microsoft\s+teams|teams)\s+(?:registration|enrollment|sign[-\s]?up)(?:\s+(?:form|page|portal|link|url))?|(?:formulario|p[aá]gina|portal|enlace|link|url)\s+de\s+(?:registro|inscripci[oó]n)\s+(?:de|para)\s+(?:webinar|seminario|seminar|meeting|sesi[oó]n\s+en\s+vivo))\s*$/i;
...
const firstRunBioLinkDescriptorPrefixPattern =
  /^(?:(?:link\s*tree|linktree|beacons(?:\s*\.?\s*ai)?|stan\s*store|koji(?:\s*\.?\s*to)?\s+(?:pages?|links?|urls?|portals?|forms?)|milkshake(?:\s*\.?\s*app)?|bio\s*\.?\s*link|bio\s+links?|bio\s+pages?|profile\s+links?|profile\s+pages?|link\s+in\s+bio|enlace\s+en\s+bio|link\s+en\s+bio|enlace\s+de\s+perfil|link\s+de\s+perfil|p[aá]gina\s+de\s+bio)(?:\s+(?:pages?|links?|urls?|portals?|forms?))?)(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i;

const firstRunBioLinkDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:link\s*tree|linktree|beacons(?:\s*\.?\s*ai)?|stan\s*store|koji(?:\s*\.?\s*to)?\s+(?:pages?|links?|urls?|portals?|forms?)|milkshake(?:\s*\.?\s*app)?|bio\s*\.?\s*link|bio\s+links?|bio\s+pages?|profile\s+links?|profile\s+pages?|link\s+in\s+bio|enlace\s+en\s+bio|link\s+en\s+bio|enlace\s+de\s+perfil|link\s+de\s+perfil|p[aá]gina\s+de\s+bio)(?:\s+(?:pages?|links?|urls?|portals?|forms?))?)\s*$/i;
