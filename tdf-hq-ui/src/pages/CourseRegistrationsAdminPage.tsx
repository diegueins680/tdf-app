...
const stripFirstRunCohortPresentationMarkers = (value: string) =>
  decodeFirstRunCohortHtmlEntities(value)
    ...
    .replace(
      /^\[(?:TODO|FIXME|DRAFT|BORRADOR|WIP|TEST|QA|UAT|DEV|DEVELOPMENT|DESARROLLO|INTERNAL|INTERN[OA]|DEMO|SAMPLE|STAGING|SANDBOX|PREVIEW|PRUEBA|MUESTRA|EJEMPLO|VISTA\s+PREVIA|ARCHIVE|ARCHIVED|ARCHIVAD[OA]|BACKUP|RESPALDO)\]\s*/gi,
      '',
    )
    .replace(
      /^(?:TODO|FIXME|DRAFT|BORRADOR|WIP|TEST|QA|UAT|DEV|DEVELOPMENT|DESARROLLO|INTERNAL|INTERN[OA]|DEMO|SAMPLE|STAGING|SANDBOX|PREVIEW|PRUEBA|MUESTRA|EJEMPLO|VISTA\s+PREVIA|ARCHIVE|ARCHIVED|ARCHIVAD[OA]|BACKUP|RESPALDO)\s*(?::|-|\u2013|\u2014)\s*/gi,
      '',
    )
    .replace(
      /\s*(?:[[(]\s*(?:TODO|FIXME|DRAFT|BORRADOR|WIP|TEST|QA|UAT|DEV|DEVELOPMENT|DESARROLLO|INTERNAL|INTERN[OA]|DEMO|SAMPLE|STAGING|SANDBOX|PREVIEW|PRUEBA|MUESTRA|EJEMPLO|VISTA\s+PREVIA|ARCHIVE|ARCHIVED|ARCHIVAD[OA]|BACKUP|RESPALDO)\s*[\])]|\s*(?::|-|\u2013|\u2014)\s*(?:TODO|FIXME|DRAFT|BORRADOR|WIP|TEST|QA|UAT|DEV|DEVELOPMENT|DESARROLLO|INTERNAL|INTERN[OA]|DEMO|SAMPLE|STAGING|SANDBOX|PREVIEW|PRUEBA|MUESTRA|EJEMPLO|VISTA\s+PREVIA|ARCHIVE|ARCHIVED|ARCHIVAD[OA]|BACKUP|RESPALDO))\s*$/gi,
      '',
    )
    .replace(firstRunListMarkerPattern, '')
...
const firstRunCopyDescriptorPrefixPattern =
  /^(?:(?:copy|duplicate)\s+of|(?:clone|cloned)\s+(?:of|from)|(?:copia|duplicad[oa]|clon(?:ad[oa])?)\s+de)\s*(?:[-:/|]\s*)?/i;

const firstRunCopyDescriptorSuffixPattern =
  /\s*(?:(?:[-:/|]\s*)|(?:\(\s*)|(?:\[\s*))?(?:copy|duplicate|clone|cloned|copia|duplicad[oa]|clon(?:ad[oa])?)\s*(?:\)|\])?\s*$/i;
