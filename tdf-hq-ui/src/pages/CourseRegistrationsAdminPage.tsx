...
const firstRunInquiryDescriptorPrefixPattern =
  /^(?:(?:course\s+)?(?:contact|inquiry|enquiry|lead)\s+(?:form|page)|formulario\s+de\s+(?:contacto|consulta|inter[eé]s)(?:\s+(?:del?\s+curso|de\s+curso|para\s+el\s+curso))?)(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i;

const firstRunInquiryDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:course\s+)?(?:contact|inquiry|enquiry|lead)\s+(?:form|page)|formulario\s+de\s+(?:contacto|consulta|inter[eé]s)(?:\s+(?:del?\s+curso|de\s+curso|para\s+el\s+curso))?)\s*$/i;
