...
const firstRunProviderFormDescriptorPrefixPattern =
  /^(?:(?:google\s+forms?)|(?:formulario\s+(?:de\s+)?google)|(?:(?:facebook|fb|meta|instagram|ig)\s+(?:lead\s+)?forms?)|(?:whats\s*app\s+(?:lead\s+)?forms?)|(?:formulario\s+(?:de\s+)?whats\s*app)|(?:microsoft|ms)\s+forms?|typeform|tally\s+forms?|jot\s*forms?|airtable\s+forms?|hubspot\s+forms?|mail\s*chimp\s+(?:sign[-\s]?up\s+)?forms?|paper\s*forms?|survey\s*monkey(?:\s+forms?)?|wufoo(?:\s+forms?)?|formstack(?:\s+forms?)?|zoho\s+forms?|gravity\s+forms?)(?:\s+(?:del|de|para\s+el|para|for))?\s*(?:[-:/|]\s*)?/i;

const firstRunProviderFormDescriptorSuffixPattern =
  /\s*(?:[-:/|]\s*)?(?:(?:google\s+forms?)|(?:formulario\s+(?:de\s+)?google)|(?:(?:facebook|fb|meta|instagram|ig)\s+(?:lead\s+)?forms?)|(?:whats\s*app\s+(?:lead\s+)?forms?)|(?:formulario\s+(?:de\s+)?whats\s*app)|(?:microsoft|ms)\s+forms?|typeform|tally\s+forms?|jot\s*forms?|airtable\s+forms?|hubspot\s+forms?|mail\s*chimp\s+(?:sign[-\s]?up\s+)?forms?|paper\s*forms?|survey\s*monkey(?:\s+forms?)?|wufoo(?:\s+forms?)?|formstack(?:\s+forms?)?|zoho\s+forms?|gravity\s+forms?)\s*$/i;
