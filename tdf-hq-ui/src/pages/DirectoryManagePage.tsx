...
function ProfileDialog({ profile, open, onClose, taxonomies, onSaved }: { profile?: ManagedDirectoryProfile; open: boolean; onClose: () => void; taxonomies?: DirectoryTaxonomies; onSaved: () => void }) {
  ...
  return <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg"><DialogTitle>{profile ? 'Editar perfil público' : 'Crear perfil público'}</DialogTitle><DialogContent><Stack spacing={2.5} pt={1}>
    ...
    {languages.map((id) => <Stack key={id} direction={{ xs: 'column', sm: 'row' }} gap={2} alignItems={{ sm: 'center' }}><Typography sx={{ minWidth: 180 }}>{taxonomies?.languages.find((item) => item.id === id)?.name ?? 'Idioma'}</Typography><FormControl fullWidth><InputLabel id={`directory-language-level-${id}`}>Dominio</InputLabel><Select labelId={`directory-language-level-${id}`} label="Dominio" value={languageLevels[id] ?? 'professional'} onChange={(event) => setLanguageLevels((current) => ({ ...current, [id]: event.target.value as typeof languageLevels[string] }))}>{[['basic','Básico'],['conversational','Conversacional'],['professional','Profesional'],['native','Nativo']].map(([value,label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</Select></FormControl></Stack>)}
    <Divider />
    <Typography variant="h6" fontWeight={800}>Disponibilidad y cobertura</Typography>
    <FormControl><InputLabel id="directory-availability-label">Disponibilidad</InputLabel><Select labelId="directory-availability-label" label="Disponibilidad" value={availabilityStatus} onChange={(event) => setAvailabilityStatus(event.target.value as typeof availabilityStatus)}>{[['available','Disponible'],['limited','Limitada'],['unavailable','No disponible'],['ask','Consultar']].map(([value,label]) => <MenuItem key={value} value={value}>{label}</MenuItem>)}</Select></FormControl>
    ...
    <FormControl required={!preservedPrimaryArea}><InputLabel id="directory-primary-city-label">Ciudad principal</InputLabel><Select labelId="directory-primary-city-label" label="Ciudad principal" value={primaryCityId} onChange={(event) => setPrimaryCityId(event.target.value)}>{cityIds.map((id) => <MenuItem key={id} value={id}>{taxonomies?.cities.find((item) => item.id === id)?.name ?? id}</MenuItem>)}</Select></FormControl>
...
function CurrencySelect({ currencies, value, onChange }: { currencies: DirectoryTaxonomyItem[]; value: string; onChange: (value: string) => void }) {
  const labelId = useId();
  return <FormControl sx={{ minWidth: 150 }}><InputLabel id={labelId}>Moneda</InputLabel><Select labelId={labelId} label="Moneda" value={value} onChange={(event) => onChange(event.target.value)}>{currencies.map((item) => <MenuItem key={item.id} value={item.id}>{item.code} · {item.symbol ?? item.name}</MenuItem>)}</Select></FormControl>;
}
