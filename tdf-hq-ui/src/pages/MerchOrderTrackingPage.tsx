...
export default function MerchOrderTrackingPage() {
  ...
  const [cancelKey, setCancelKey] = useState(() => createMerchIdempotencyKey('cancel-order'));
  ...
  const cancel = useMutation({
    mutationFn: () => Merch.cancelUnpaidOrder(orderId, stored!.token, cancelReason.trim(), cancelKey),
    onSuccess: async () => {
      setCancelReason('');
      setCancelKey(createMerchIdempotencyKey('cancel-order'));
      await order.refetch();
    },
  });
