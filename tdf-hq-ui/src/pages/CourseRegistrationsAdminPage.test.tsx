...
describe('CourseRegistrationsAdminPage', () => {
  ...
  it('strips webinar and meeting registration wrappers from first-run cohort copy', async () => {
    const titles = [
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/Zoom|Google Meet|webinar registration|sign-up link/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips community-channel wrappers from first-run cohort copy', async () => {
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/WhatsApp group|Grupo de WhatsApp|Discord community|Telegram chat|comunidad de Telegram/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips course-platform checkout wrappers from first-run cohort copy', async () => {
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/Kajabi|Teachable|Thinkific|Hotmart|Podia|LearnWorlds|ClickFunnels|GoHighLevel|Kartra|Systeme\.io|Mighty Networks|Skool|Circle\.so/i);
  ...
  it('strips sales-page descriptors from first-run cohort copy', async () => {
    const titles = [
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/sales page|order form|purchase page|p[aá]gina de ventas|formulario de compra|enlace de venta/i);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
        ).toBe('Abrir formulario público de Beatmaking 101');
