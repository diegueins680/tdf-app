...
describe('CourseRegistrationsAdminPage', () => {
  ...
  it('ignores placeholder phone labels so first-time contact guidance stays accurate', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
        crEmail: null,
        crPhoneE164: 'Sin número',
      }),
    ]);
    ...
    await waitForExpectation(() => {
      ...
      expect(getButtonByAriaLabel(container, 'Cambiar estado para Ada Lovelace').textContent?.trim()).toBe('Pendiente de pago');
      expect(countOccurrences(container, 'Contacto pendiente en esta inscripción.')).toBe(1);
      expect(container.textContent).not.toContain('Sin número');
      expect(container.textContent).not.toContain('Sin correo ni teléfono');
      expect(container.querySelector('[data-testid="course-registration-current-view-summary"]')).toBeNull();
      expect(container.querySelector('[data-testid="course-registration-list-utilities"]')).toBeNull();
    });
  ...
  it('treats unresolved email and WhatsApp placeholders as missing contact', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
  ...
  it('treats unanswered contact placeholders as missing contact', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
        crEmail: 'No responde',
        crPhoneE164: 'No contesta',
      }),
    ]);
    ...
    await waitForExpectation(() => {
      ...
      expect(getButtonByAriaLabel(container, 'Cambiar estado para Ada Lovelace').textContent?.trim()).toBe('Pendiente de pago');
      expect(countOccurrences(container, 'Contacto pendiente en esta inscripción.')).toBe(1);
      expect(container.textContent).not.toContain('No responde');
      expect(container.textContent).not.toContain('No contesta');
      expect(container.textContent).not.toContain('Sin correo ni teléfono');
      expect(container.querySelector('[data-testid="course-registration-current-view-summary"]')).toBeNull();
      expect(container.querySelector('[data-testid="course-registration-list-utilities"]')).toBeNull();
    });
  ...
  it('treats unspecified contact placeholders from form exports as missing contact', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
        crEmail: 'None provided',
        crPhoneE164: 'Sin especificar',
      }),
    ]);
    ...
    await waitForExpectation(() => {
      ...
      expect(getButtonByAriaLabel(container, 'Cambiar estado para Ada Lovelace').textContent?.trim()).toBe('Pendiente de pago');
      expect(countOccurrences(container, 'Contacto pendiente en esta inscripción.')).toBe(1);
      expect(container.textContent).not.toContain('None provided');
      expect(container.textContent).not.toContain('Sin especificar');
      expect(container.textContent).not.toContain('Sin correo ni teléfono');
      expect(container.querySelector('[data-testid="course-registration-current-view-summary"]')).toBeNull();
      expect(container.querySelector('[data-testid="course-registration-list-utilities"]')).toBeNull();
    });
  ...
  it('treats English no-contact export labels as missing contact', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
        crEmail: 'No e-mail',
        crPhoneE164: 'No phone',
      }),
    ]);
    ...
    await waitForExpectation(() => {
      ...
      expect(getButtonByAriaLabel(container, 'Cambiar estado para Ada Lovelace').textContent?.trim()).toBe('Pendiente de pago');
      expect(countOccurrences(container, 'Contacto pendiente en esta inscripción.')).toBe(1);
      expect(container.textContent).not.toContain('No e-mail');
  ...
  it('clears stale follow-up feedback when opening another follow-up composer', async () => {
    getRegistrationDossierMock.mockResolvedValue(
      buildDossier({
        crdFollowUps: [buildFollowUp()],
      }),
    ...
    await waitForExpectation(() => {
      expect(getButtonByText(document.body, 'Agregar seguimiento')).toBeTruthy();
      expect(document.body.textContent).not.toContain('Seguimiento guardado.');
    });
    ...
    await act(async () => {
      setInputValue(getInputByLabel(document.body, 'Nota de seguimiento'), 'Nuevo seguimiento sin banner viejo.');
      await flushPromises();
      await flushPromises();
    });
    ...
    await act(async () => {
      clickButton(getButtonByText(document.body, 'Guardar seguimiento'));
      await flushPromises();
      await flushPromises();
    });

    await waitForExpectation(() => {
      expect(document.body.textContent).toContain('Seguimiento guardado.');
      expect(getButtonByText(document.body, 'Agregar seguimiento')).toBeTruthy();
    });
  ...
  it('keeps a URL-only cohort slug as a readable filter instead of treating it as a configured form', async () => {
    ...
    await waitForExpectation(() => {
      ...
      expect(countButtonsByText(container, 'Quitar filtro de cohorte')).toBe(0);
      expect(countButtonsByText(container, 'Mostrar todas las cohortes')).toBe(0);
      expect(countButtonsByText(container, 'Mostrar todos los formularios')).toBe(0);
    });

    listRegistrationsMock.mockClear();

    await act(async () => {
      clickButton(getButtonByText(container, 'Quitar filtro de formulario'));
      await flushPromises();
      await flushPromises();
    });
  ...
  it('deduplicates repeated registrations before showing row actions or export utilities', async () => {
    ...
    await waitForExpectation(() => {
      ...
      expect(container.querySelectorAll(`button[aria-label="${paymentStatusIconButtonAriaLabel('Ada Lovelace')}"]`)).toHaveLength(1);
  ...
  it('lets admins search missing email or phone states without rendering placeholder row labels', async () => {
    listRegistrationsMock.mockResolvedValue([
      buildRegistration({
        crId: 101,
        crFullName: 'Nina Email',
        crEmail: 'nina@example.com',
        crPhoneE164: null,
      }),
      buildRegistration({
    ...
    await waitForExpectation(() => {
      ...
      expect(getDossierTriggers(container)).toHaveLength(9);
      expect(container.textContent).not.toContain('Sin correo');
      expect(container.textContent).not.toContain('Sin teléfono');
      expect(container.textContent).not.toContain('Sin correo ni teléfono');
    });
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'sin teléfono');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(1);
      expect(container.textContent).toContain('Nina Email');
      expect(container.textContent).not.toContain('Paco Telefono');
      expect(container.textContent).not.toContain('No hay coincidencias');
      expect(container.textContent).not.toContain('Sin teléfono');
      expect(listRegistrationsMock).not.toHaveBeenCalled();
    });
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'sin correo');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(1);
      expect(container.textContent).toContain('Paco Telefono');
      expect(container.textContent).not.toContain('Nina Email');
      expect(container.textContent).not.toContain('No hay coincidencias');
      expect(container.textContent).not.toContain('Sin correo');
      expect(listRegistrationsMock).not.toHaveBeenCalled();
    });
  ...
  it('treats form-provider sources as default public-form plumbing in busy lists', async () => {
    const defaultSources = [
      ...
      'substack_form',
      'crm_lead_form',
      'crm_registration_page',
      'crm_intake_form',
      'wati_lead_form',
      'kommo_registration_page',
      'pipedrive_lead_form',
      'meta_instant_form',
  ...
  it('treats import and migration sources as admin plumbing in busy lists', async () => {
    const defaultSources = [
      'manual_import',
      'bulk_upload',
      'csv_import',
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'manual import');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(0);
      expect(container.textContent).toContain(
        `No hay coincidencias para "manual import" en las ${defaultSources.length} inscripciones cargadas.`,
      );
      expect(listRegistrationsMock).not.toHaveBeenCalled();
    });
  ...
  it('treats static form backend sources as default public-form plumbing in busy lists', async () => {
    const defaultSources = [
      'netlify_forms',
      'formkeep',
      'formkeep_form',
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'netlify');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(0);
      expect(container.textContent).toContain(
        `No hay coincidencias para "netlify" en las ${defaultSources.length} inscripciones cargadas.`,
      );
      expect(listRegistrationsMock).not.toHaveBeenCalled();
    });
  ...
  it('ignores placeholder acquisition metadata in busy-list search prompts', async () => {
    listRegistrationsMock.mockResolvedValue([
      ...
      buildRegistration({
        ...
        crHowHeard: 'not_tracked',
        crUtmSource: 'por_actualizar',
        crUtmMedium: 'por_confirmar',
        crUtmCampaign: 'no_campaign',
        crUtmContent: 'sin_contenido',
      }),
      ...buildRegistrations(7, (index) => ({
        crId: 200 + index,
        crPartyId: 40 + index,
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'sin campaña');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(0);
      expect(container.textContent).toContain(
        'No hay coincidencias para "sin campaña" en las 9 inscripciones cargadas.',
      );
    ...
    await act(async () => {
      setInputValue(getInputByLabel(container, localSearchLabel), 'not tracked');
      await flushPromises();
      await flushPromises();
    });
    ...
    await waitForExpectation(() => {
      expect(getDossierTriggers(container)).toHaveLength(0);
      expect(container.textContent).toContain(
        'No hay coincidencias para "not tracked" en las 9 inscripciones cargadas.',
      );
      expect(container.textContent).not.toContain('Coincide con origen o campaña.');
      expect(container.textContent).not.toContain('Coinciden con origen o campaña.');
      expect(listRegistrationsMock).not.toHaveBeenCalled();
  ...
  it('clears stale status feedback when the admin starts another row action', async () => {
    ...
    await waitForExpectation(() => {
      expect(getButtonByAriaLabel(container, paymentStatusIconButtonAriaLabel('Ada Lovelace'))).toBeTruthy();
      expect(getButtonByAriaLabel(container, paymentStatusIconButtonAriaLabel('Grace Hopper'))).toBeTruthy();
    });
    ...
    await act(async () => {
      clickButton(getButtonByAriaLabel(container, paymentStatusIconButtonAriaLabel('Grace Hopper')));
      await flushPromises();
      await flushPromises();
    });
  ...
  it('keeps busy-list search focused when cohort filters are unavailable', async () => {
    ...
    await waitForExpectation(() => {
      ...
      expect(container.textContent).not.toContain('volverá al reintentar');
      expect(searchInput.getAttribute('placeholder')).toBe('Nombre o contacto');
      expect(hasLabel(container, cohortFilterLabel)).toBe(false);
      expect(getDossierTriggers(container)).toHaveLength(9);
    });
  ...
  it('strips generated untitled-form wrappers from first-run course labels', async () => {
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toMatch(/untitled|sin t[ií]tulo|nuevo formulario/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(formAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(formAction?.getAttribute('title')).toBe(
  ...
  it('strips numbered generic form wrappers from first-run course labels', async () => {
    const titles = [
      'Form 1 - Beatmaking 101',
      'Beatmaking 101 - Form #2',
      'Formulario 3 - Beatmaking 101',
      'Beatmaking 101 - Formulario número 4',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/form\s*#?\d|formulario\s*(?:\d|n[uú]mero)/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(formAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(formAction?.getAttribute('title')).toBe(
          'Abrir formulario público de Beatmaking 101 en una pestaña nueva',
  ...
  it('strips generated response-sheet wrappers from first-run course labels', async () => {
    const titles = [
      'Form responses 1 - Beatmaking 101',
      'Responses - Beatmaking 101',
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/form responses|respuestas del formulario|google forms responses/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(formAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(formAction?.getAttribute('title')).toBe(
          'Abrir formulario público de Beatmaking 101 en una pestaña nueva',
  ...
  it('strips common form-provider descriptors from first-run cohort copy', async () => {
    const titles = [
      ...
      'HubSpot landing page - Beatmaking 101',
      'Beatmaking 101 - HubSpot landing page',
  ...
  it('strips automation plumbing wrappers from first-run cohort copy', async () => {
    const titles = [
      'Zapier webhook - Beatmaking 101',
      'Zapier registration automation for Beatmaking 101',
      'Webhook de Zapier para Beatmaking 101',
      'Make.com scenario - Beatmaking 101',
      'Beatmaking 101 - Make.com workflow',
      'n8n registration flow - Beatmaking 101',
      'Beatmaking 101 - Pabbly Connect integration',
      'Integrately automation for Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/Zapier|Webhook|Make\.com|n8n|Pabbly|Integrately|automation|workflow|scenario|integration/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips internal data-source wrappers from first-run cohort copy', async () => {
    const titles = [
      'Airtable registration table - Beatmaking 101',
  ...
  it('strips import-upload wrappers from first-run cohort copy', async () => {
    const titles = [
      'CSV import - Beatmaking 101',
      'Beatmaking 101 - carga CSV',
    ];
  ...
  it('strips no-code landing provider wrappers from first-run cohort copy', async () => {
    const titles = [
      'Carrd form - Beatmaking 101',
      'Beatmaking 101 - Carrd link',
      'Framer landing page - Beatmaking 101',
  ...
  it('strips community-channel wrappers from first-run cohort copy', async () => {
    const titles = [
      ...
      'Beatmaking 101 - Discord invite link',
      'Enlace de invitación al grupo de WhatsApp para Beatmaking 101',
    ];
  ...
  it('strips course-platform checkout wrappers from first-run cohort copy', async () => {
    const titles = [
      ...
      'GHL registration funnel - Beatmaking 101',
      'GoHighLevel registration funnel for Beatmaking 101',
      'HighLevel form - Beatmaking 101',
      'LeadConnector registration funnel for Beatmaking 101',
      ...
      'Udemy course page - Beatmaking 101',
  ...
  it('strips ticketing wrappers from first-run cohort copy', async () => {
    const titles = [
      'Ticketing portal - Beatmaking 101',
      'Box office page for Beatmaking 101',
      'Page for ticketing - Beatmaking 101',
      'Boletería del curso - Beatmaking 101',
      'Beatmaking 101 - portal de boletería',
      'Beatmaking 101 - página de taquilla',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/ticketing|box office|boleter[ií]a|taquilla/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips scheduling-provider reservation descriptors from first-run cohort copy', async () => {
    const titles = [
      ...
      'OnceHub appointment page for Beatmaking 101',
      'Appointlet reservation link - Beatmaking 101',
      'Book Like A Boss booking page for Beatmaking 101',
    ];
  ...
  it('strips launch-page wrappers from first-run cohort copy', async () => {
    const titles = [
      'Course launch page - Beatmaking 101',
      'Launch campaign for Beatmaking 101',
      'Beatmaking 101 - launch funnel',
      'Pagina de lanzamiento - Beatmaking 101',
      'Campaña de lanzamiento - Beatmaking 101',
      'Beatmaking 101 - lanzamiento del curso',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/launch|lanzamiento/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips registration-link descriptors from first-run cohort copy', async () => {
    const titles = [
      ...
      'Vínculo público de inscripción - Beatmaking 101',
      'Beatmaking 101 - vinculo público para inscripción',
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toMatch(/(?:link|enlace|v[ií]nculo|url)\s+de\s+inscripci[oó]n/i);
        expect(emptyState?.textContent).not.toMatch(/(?:link|enlace|v[ií]nculo|url)\s+p[uú]blic[oa]s?\s+(?:de|para)\s+inscripci[oó]n/i);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
        ).toBe('Abrir formulario público de Beatmaking 101');
  ...
  it('strips payment and checkout descriptors from first-run cohort copy', async () => {
    const titles = [
      ...
      'Hotmart payment link - Beatmaking 101',
      'Kiwify checkout - Beatmaking 101',
      'Beatmaking 101 - Kiwify payment link',
  ...
  it('strips landing-page wrappers from first-run cohort copy', async () => {
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toMatch(/\blanding\b|p[aá]gina landing/i);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.textContent?.trim(),
        ).toBe(initialEmptyStateFormActionLabel);
  ...
  it('strips roster and student-list wrappers from first-run cohort copy', async () => {
    const titles = [
      'Course roster - Beatmaking 101',
      'Beatmaking 101 - student roster',
      'Lista de estudiantes del curso - Beatmaking 101',
      'Beatmaking 101 - listado de alumnos',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/roster|student list|lista de estudiantes|listado de alumnos/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(formAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
  ...
  it('strips attendance and check-in wrappers from first-run cohort copy', async () => {
    const titles = [
      'Attendance form - Beatmaking 101',
      'Course attendance sheet for Beatmaking 101',
      'Beatmaking 101 - student check-in form',
      'Lista de asistencia - Beatmaking 101',
      'Beatmaking 101 - registro de asistencia',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/attendance|check[-\s]?in|lista de asistencia|registro de asistencia/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(formAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
  ...
  it('strips orientation-form wrappers from first-run cohort copy', async () => {
    const titles = [
      'Student orientation form for Beatmaking 101',
      'Course orientation portal - Beatmaking 101',
      'Formulario de orientación para Beatmaking 101',
      'Beatmaking 101 - orientation checklist',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/orientation\s+(?:form|portal|checklist)|formulario de orientaci[oó]n/i);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
        ).toBe('Abrir formulario público de Beatmaking 101');
  ...
  it('strips tuition-assistance wrappers from first-run cohort copy', async () => {
    const titles = [
      'Tuition assistance form for Beatmaking 101',
      'Payment assistance request - Beatmaking 101',
      'Beatmaking 101 - tuition support application',
      'Solicitud de ayuda de matrícula - Beatmaking 101',
      'Beatmaking 101 - formulario de facilidades de pago',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(
          /tuition (?:assistance|support)|payment assistance|ayuda de matr[ií]cula|facilidades de pago/i,
        );
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
  ...
  it('strips generated list markers from first-run cohort copy', async () => {
    const titles = [
      '1. Registration page - Beatmaking 101',
      '2) Formulario público - Beatmaking 101',
      '- Beatmaking 101',
      '* Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/1\. Registration|2\) Formulario|- Beatmaking|\* Beatmaking/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('falls back to the readable course name when first-run form titles are copied URLs', async () => {
    const titles = [
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/https?:\/\/|www\.|forms\.gle|example\.com|tdfrecords\.com/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips invisible formatting marks from first-run cohort copy', async () => {
    listCohortsMock.mockResolvedValue([
      { ccSlug: 'beatmaking-101', ccTitle: 'Beat\u200bmaking\u2060 101' },
    ]);
    listRegistrationsMock.mockResolvedValue([]);
  ...
  it('strips generated-state markers from first-run cohort copy', async () => {
    const titles = [
      '[TEST] Typeform registration page - Beatmaking 101',
      'DEMO: Formulario de Tally para Beatmaking 101',
      '[QA] Typeform registration page - Beatmaking 101',
      'UAT - Google Forms - Beatmaking 101',
      'Dev - Typeform registration page - Beatmaking 101',
      'Interno: Formulario de Tally para Beatmaking 101',
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toMatch(
          /TEST|DEMO|QA|UAT|Dev|Interno|Sample|Preview|Sandbox|Vista previa|PRUEBA|ARCHIVED|Archivado|Backup|Typeform|Tally|Google Forms|registration page/i,
        );
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
  ...
  it('strips internal admin workflow wrappers from first-run cohort copy', async () => {
    const titles = [
      'Admin review queue - Beatmaking 101',
      'Beatmaking 101 - internal approval tracker',
      'Ops board for Beatmaking 101',
      'Cola de revisión - Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/review queue|approval tracker|ops board|cola de revisi[oó]n/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips urgency offer wrappers from first-run cohort copy', async () => {
    const titles = [
      'Last chance enrollment page - Beatmaking 101',
      'Beatmaking 101 - registration deadline',
      'Final call signup form for Beatmaking 101',
      'Últimos cupos - Beatmaking 101',
      'Cierre de inscripciones - Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(
          /last[-\s]?chance|registration deadline|final call|[uú]ltimos?\s+cupos?|cierre de inscripciones/i,
        );
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
  ...
  it('strips apply-now wrappers from first-run cohort copy', async () => {
    const titles = [
      ...
      'Start application - Beatmaking 101',
      'Begin enrollment form for Beatmaking 101',
      'Beatmaking 101 - complete your registration',
      'Beatmaking 101 - enroll now',
      'Beatmaking 101 - sign up here',
      'Inscríbete ahora - Beatmaking 101',
      'Regístrate aquí - Beatmaking 101',
      'Inicia tu inscripción - Beatmaking 101',
      'Beatmaking 101 - completa la matrícula',
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(
          /apply now|register here|register for|start application|begin enrollment|complete your registration|enroll now|sign up here|inscr[ií]bete ahora|inscr[ií]bete al curso|inicia tu inscripci[oó]n|completa la matr[ií]cula|reg[ií]strate aqu[ií]|reg[ií]strate ahora|reg[ií]strate al curso|inscr[ií]bete aqu[ií]|the course/i,
        );
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips course-info page wrappers from first-run cohort copy', async () => {
    const titles = [
      'Course details page - Beatmaking 101',
      'Beatmaking 101 - course information page',
      'Página de información del curso - Beatmaking 101',
      'Beatmaking 101 - detalles del curso',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(copy).toContain(singleCohortInitialEmptyStateMessage);
        expect(copy).not.toContain(title);
        expect(copy).not.toMatch(/course\s+(?:details?|information|info)|p[aá]gina de informaci[oó]n|detalles del curso/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips schedule-page wrappers from first-run cohort copy', async () => {
    const titles = [
      ...
      'Beatmaking 101 - class timetable page',
      'Página de horarios del curso - Beatmaking 101',
      'Beatmaking 101 - calendario de clases',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(copy).toContain(singleCohortInitialEmptyStateMessage);
        expect(copy).not.toContain(title);
        expect(copy).not.toMatch(/schedule|calendar|timetable|horarios?|calendarios?|cronogramas?/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(countOccurrences(emptyState!, 'formulario público')).toBe(1);
        expect(
          emptyState?.querySelector<HTMLAnchorElement>('a[href="/inscripcion/beatmaking-101"]')?.getAttribute('aria-label'),
  ...
  it('strips workspace brand wrappers from first-run cohort copy', async () => {
    const titles = [
      'TDF Records - Beatmaking 101',
      'TDF Academy: Beatmaking 101',
      'Beatmaking 101 | TDF Records',
      'The Domo Factory / Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/TDF Records|TDF Academy|The Domo Factory/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(publicFormAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
      });
  ...
  it('strips bare Spanish registration nouns from first-run course labels', async () => {
    const titles = [
      'Inscripciones Beatmaking 101',
      'Registro: Beatmaking 101',
      'Matrículas - Beatmaking 101',
      'Solicitudes Beatmaking 101',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(
          /Todavía no hay inscripciones para (Inscripciones|Registro|Matr[ií]culas|Solicitudes)/,
        );
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(publicFormAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
  ...
  it('strips invite-only wrappers from first-run cohort copy', async () => {
    const titles = [
      'Invite-only registration page - Beatmaking 101',
      'Invitation form for Beatmaking 101',
      'Invitación al curso - Beatmaking 101',
      'Beatmaking 101 - acceso por invitación',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/invite[-\s]?only|invitation form|invitaci[oó]n|acceso por invitaci[oó]n/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(publicFormAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
      });
  ...
  it('strips assignment-submission wrappers from first-run cohort copy', async () => {
    const titles = [
      'Homework submission form - Beatmaking 101',
      'Beatmaking 101 - assignment upload page',
      'Entrega de tarea - Beatmaking 101',
      'Beatmaking 101 - subida de proyecto',
    ];
    ...
    for (const title of titles) {
      ...
      await waitForExpectation(() => {
        ...
        expect(emptyState?.textContent).toContain(singleCohortInitialEmptyStateMessage);
        expect(emptyState?.textContent).not.toContain(title);
        expect(emptyState?.textContent).not.toMatch(/homework|assignment upload|entrega de tarea|subida de proyecto/i);
        expect(countOccurrences(emptyState!, 'Beatmaking 101')).toBe(1);
        expect(publicFormAction?.getAttribute('aria-label')).toBe('Abrir formulario público de Beatmaking 101');
        expect(emptyState?.querySelectorAll('a')).toHaveLength(1);
      });
  ...
  it('strips certificate artifact wrappers from first-run cohort copy', async () => {
    const titles = [
  ...
  it('folds tiny default-list counts into the current view instead of adding a utility row', async () => {
    ...
    await waitForExpectation(() => {
      ...
      expect(
        container.querySelector<HTMLElement>('[data-testid="course-registration-single-choice-context"]')?.textContent,
      ).toContain(paymentWorkflowDossierScopeHint);
      expect(
        container.querySelector<HTMLElement>('[data-testid="course-registration-single-choice-context"]')?.textContent,
      ).not.toContain(dossierLinkScopeHint);
      expect(container.querySelector('[data-testid="course-registration-list-utilities"]')).toBeNull();
      expect(getButtonByAriaLabel(container, 'Abrir expediente de Ada Lovelace')).toBeTruthy();
      expect(countButtonsByText(container, paymentStatusMenuButtonLabel)).toBe(0);
      expect(countButtonsByText(container, openPaymentWorkflowLabel)).toBe(0);
