...
jest.unstable_mockModule('./LiveSessionIntakePage', () => ({ LiveSessionIntakeForm: () => <input aria-label="Nombre de banda" /> }));
...
it('validates the existing session endpoint even with a same-origin API and enables only a persisted account', async () => {
 ...
 expect(fetchMock).toHaveBeenCalledWith('/session', expect.objectContaining({ headers: { Authorization: 'Bearer synthetic-code' }, signal: expect.any(AbortSignal) }));
 expect(transient).toHaveBeenCalledWith('synthetic-code');
});
...
it('ignores an old successful validation after the code changes', async () => {
 ...
 await waitFor(() => expect(screen.getByLabelText('Nombre de banda')).toBeEnabled());
 expect(transient).toHaveBeenCalledWith('synthetic-new');
});
it('retains recoverable form input while a changed code is unverified', async () => {
 ...
 const band=screen.getByLabelText('Nombre de banda');await waitFor(()=>expect(band).toBeEnabled());
 fireEvent.change(band,{target:{value:'Banda sintética'}});enter('');
 expect(band).toBeDisabled();expect(band).toHaveValue('Banda sintética');
 expect(transient).toHaveBeenLastCalledWith(null);
});
