...
- Implemented an internal campaign operations dashboard for `TDF Sessions x Domo Pululahua` at `/estudio/campanas/tdf-sessions-domo`. It tracks the 21-day calendar, video asset statuses, ad metrics, budget split, campaign copy, WhatsApp templates, and weighted band selection scoring with local browser persistence.
- Fixed the GitHub Actions `quality` failure by restoring inline exports expected by the continuous-improvement-loop tests, correcting the UI Jest ts-jest preset, tightening lint/test config, and hardening backend validators uncovered by the full Haskell test suite. Verified `npm run quality` passes locally.
