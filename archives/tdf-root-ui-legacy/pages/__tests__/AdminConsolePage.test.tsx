...
describe('AdminConsolePage', () => {
  ...
  it('keeps combined API and database fallback cards from duplicating service health', async () => {
    mockConsolePreview.mockResolvedValue({
      status: 'preview',
      cards: [
        {
          cardId: 'fallback-api-database',
          title: 'API and database',
          body: ['Confirm uptime before changing admin permissions.'],
        },
        {
          cardId: 'fallback-api-base-datos',
          title: 'API y base de datos',
          body: ['Confirma disponibilidad antes de ajustar accesos administrativos.'],
        },
        {
          cardId: 'fallback-api-db',
          title: 'API / DB',
          body: ['Review connectivity before loading sample admin data.'],
        },
      ],
    });
    ...
    expect(screen.queryByText('Módulos adicionales')).not.toBeInTheDocument();
    expect(screen.queryByText('API and database')).not.toBeInTheDocument();
    expect(screen.queryByText('API y base de datos')).not.toBeInTheDocument();
    expect(screen.queryByText('API / DB')).not.toBeInTheDocument();
    expect(screen.queryByText(/Confirm uptime before changing admin permissions\./i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Confirma disponibilidad antes de ajustar accesos administrativos\./i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Review connectivity before loading sample admin data\./i)).not.toBeInTheDocument();
  });
  ...
  it('keeps overview and center-labeled built-in admin titles out of optional modules', async () => {
    mockConsolePreview.mockResolvedValue({
      status: 'preview',
      cards: [
        {
          cardId: 'fallback-service-health-overview',
          title: 'Service health overview',
          body: ['Review readiness signals before editing administrative access.'],
        },
        {
          cardId: 'fallback-users-roles-summary',
          title: 'Users and roles summary',
          body: ['Review team access coverage before changing permissions.'],
        },
        {
          cardId: 'fallback-recent-audit-center',
          title: 'Recent audit center',
          body: ['Check recent workspace changes before repeating an admin action.'],
        },
      ],
    });
    ...
    expect(screen.queryByText('Módulos adicionales')).not.toBeInTheDocument();
    expect(screen.queryByText('Service health overview')).not.toBeInTheDocument();
    expect(screen.queryByText('Users and roles summary')).not.toBeInTheDocument();
    expect(screen.queryByText('Recent audit center')).not.toBeInTheDocument();
    expect(
      screen.queryByText(/Review readiness signals before editing administrative access\./i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/Review team access coverage before changing permissions\./i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/Check recent workspace changes before repeating an admin action\./i),
    ).not.toBeInTheDocument();
  });
