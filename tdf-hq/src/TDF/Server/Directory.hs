...
directoryTaxonomies mLocale =
  jsonOne err500
    "WITH requested AS (SELECT CASE WHEN ?::text IN ('es','en','pt') THEN ?::text ELSE 'es' END locale) SELECT jsonb_build_object('locale',requested.locale,'professions',(SELECT jsonb_agg(jsonb_build_object('id',item.id,'code',item.code,'slug',item.current_slug,'name',coalesce(translation.name,CASE WHEN requested.locale='en' THEN item.name_en ELSE item.name_es END),'parentId',item.parent_id) ORDER BY item.sort_order,item.code) FROM profession item LEFT JOIN catalog_item_translation translation ON translation.entity_id=item.id AND translation.locale_id=(SELECT id FROM locale_reference WHERE code=requested.locale LIMIT 1) WHERE item.active),'classifiedCategories',(SELECT jsonb_agg(jsonb_build_object('id',item.id,'code',item.code,'slug',item.current_slug,'name',CASE WHEN requested.locale='en' THEN item.name_en ELSE item.name_es END,'requirements',item.requirements) ORDER BY item.sort_order) FROM classified_category item WHERE item.active),'compensationTypes',(SELECT jsonb_agg(jsonb_build_object('id',item.id,'code',item.code,'name',CASE WHEN requested.locale='en' THEN item.name_en ELSE item.name_es END) ORDER BY item.sort_order) FROM compensation_type item WHERE item.active),'instruments',(SELECT jsonb_agg(jsonb_build_object('id',id,'code',code,'name',CASE WHEN requested.locale='en' THEN name_en ELSE name_es END) ORDER BY sort_order) FROM instrument WHERE active),'genres',(SELECT jsonb_agg(jsonb_build_object('id',id,'code',code,'name',CASE WHEN requested.locale='en' THEN name_en ELSE name_es END) ORDER BY sort_order) FROM genre WHERE active),'cities',(SELECT jsonb_agg(jsonb_build_object('id',id,'code',code,'name',CASE WHEN requested.locale='en' THEN name_en ELSE name_es END,'countryId',country_id,'latitude',latitude,'longitude',longitude) ORDER BY sort_order) FROM city_reference WHERE active)) FROM requested"
