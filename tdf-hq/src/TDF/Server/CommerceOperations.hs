...
loadAmountComponentSummaries = do
  rows <- rawSql
    "SELECT checkout.environment, component.component_type, component.source, component.currency,\
    \ COUNT(*)::bigint, COALESCE(SUM(component.amount_minor), 0)::bigint\
  ...
  pure
    ...
    | ( Single environment, Single componentType, Single source, Single currency
