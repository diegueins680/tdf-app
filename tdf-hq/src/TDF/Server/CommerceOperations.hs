...
loadPaymentIntentSummaries = do
  rows <- rawSql
    "SELECT checkout.environment, intent.status, intent.currency, COUNT(*)::bigint,\
    \ COALESCE(SUM(intent.amount_minor), 0)::bigint,\
    \ COALESCE(SUM(intent.authorized_minor), 0)::bigint,\
    \ COALESCE(SUM(intent.captured_minor), 0)::bigint,\
    \ COALESCE(SUM(intent.refunded_minor), 0)::bigint\
