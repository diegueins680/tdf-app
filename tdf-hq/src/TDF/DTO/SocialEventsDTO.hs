...
normalizeOptionalText :: Maybe Text -> Parser (Maybe Text)
normalizeOptionalText Nothing = pure Nothing
normalizeOptionalText (Just rawValue) =
    let trimmed = T.strip rawValue
     in pure (if T.null trimmed then Nothing else Just trimmed)
...
instance FromJSON EventLiveBroadcastCreateDTO where
    parseJSON = withObject "EventLiveBroadcastCreateDTO" $ \o -> do
        ...
        rejectNullObjectFields
            ...
            o
        artistName <- o .:? "elbCreateArtistName" >>= normalizeOptionalText
        broadcasterName <- o .:? "elbCreateBroadcasterName" >>= normalizeOptionalText
        broadcasterPartyId <- o .:? "elbCreateBroadcasterPartyId" >>= normalizeOptionalText
        title <- o .:? "elbCreateTitle" >>= normalizeOptionalText
        description <- o .:? "elbCreateDescription" >>= normalizeOptionalText
        quality <- o .:? "elbCreateQuality" >>= normalizeOptionalText
        EventLiveBroadcastCreateDTO
            <$> (o .: "elbCreateArtistId" >>= normalizeRequiredText "elbCreateArtistId")
            <*> pure artistName
            <*> pure broadcasterName
            <*> pure broadcasterPartyId
            <*> pure title
            <*> pure description
            <*> pure quality
...
instance FromJSON EventLiveBroadcastEndDTO where
    parseJSON = withObject "EventLiveBroadcastEndDTO" $ \o -> do
        ...
        rejectNullObjectFields
            ...
            o
        EventLiveBroadcastEndDTO
            <$> (o .:? "elbEndBroadcasterPartyId" >>= normalizeOptionalText)
...
normalizePositiveIdText fieldName rawValue =
    let trimmed = T.strip rawValue
     in if T.null trimmed || not (T.all isAsciiDigitText trimmed)
            ...
            else case readMaybe (T.unpack trimmed) :: Maybe Int64 of
            ...
            then fail "Promo code must not be empty"
                ...
                if T.length code > 50
                    then fail "Promo code must be 50 characters or fewer"
                        ...
                        if not (T.all isValidPromoChar code)
...
instance ToJSON TicketPurchaseWithPromoDTO where
    toJSON TicketPurchaseWithPromoDTO{..} =
        object
