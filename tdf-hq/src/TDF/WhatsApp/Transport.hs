...
missingConfigMessage WhatsAppEnv{waToken, waPhoneId, waApiVersion} =
  let missingPieces =
        [ ("token",      ["WHATSAPP_TOKEN", "WA_TOKEN"])                                    | isNothing waToken      ] ++
        [ ("phoneId",    ["WHATSAPP_PHONE_NUMBER_ID", "WA_PHONE_ID"])                       | isNothing waPhoneId    ] ++
        [ ("apiVersion", ["WHATSAPP_API_VERSION", "WA_GRAPH_API_VERSION", "WA_API_VERSION"]) | isNothing waApiVersion ]
