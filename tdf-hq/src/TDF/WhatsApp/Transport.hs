...
sendWhatsAppTextIO env@WhatsAppEnv{waToken = Just tok, waPhoneId = Just pid, waApiVersion = mVersion} phone msg = do
...
missingConfigMessage WhatsAppEnv{waToken, waPhoneId} =
  let missingPieces =
      ...
      pieceToText (name, envs) =
        T.concat [name, " (expected env vars: ", T.intercalate ", " (map T.pack envs), ")"]
      details =
