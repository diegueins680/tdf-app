...
BEGIN
  IF EXISTS(SELECT 1 FROM identity_merge_history) OR EXISTS(SELECT 1 FROM identity_contact_request)
    OR EXISTS(SELECT 1 FROM identity_reconciliation_case) OR EXISTS(SELECT 1 FROM identity_complementary_link) THEN
...
DROP FUNCTION IF EXISTS identity_unlink_parties(uuid);
DROP FUNCTION IF EXISTS identity_link_parties(uuid,uuid,text);
DROP FUNCTION IF EXISTS identity_link_plan(uuid);
...
DROP TABLE identity_complementary_link;
