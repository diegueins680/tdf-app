...
main = runStdoutLoggingT $ do
    ...
    liftIO $ do
        ...
        putStrLn "CORS: enabled for development (origin *)."
        ...
        -- Start the server with permissive CORS for dev service calls
        let app :: Application
            app = cors (const $ Just corsPolicy) $ serve api $ server pool
        run 8080 app
