...
The operation locks the pilot and candidate, verifies that the candidate belongs to the approved pilot, resolves or creates provider-linked venue and artist entities, reuses an unambiguous matching event when present, inserts the provider event reference, links the candidate, and appends the `materialized` audit change in one transaction. A newly created event is owned by the authenticated strict administrator that initiated materialization, so later manual corrections remain manageable; reused events keep their existing owner and gain only missing confirmed lineup links. Until a structured provider exposes its native entity IDs, materialization records deterministic `event-research:*` venue and artist references. A later discovery refresh attaches the provider's native ID to the same entity only when the same-provider synthetic reference and normalized identity resolve uniquely; venue matching additionally requires city and a confirmed country. Ambiguity aborts the transaction instead of guessing or creating duplicates. Unpublished materializations store a `materialization_draft:` provider status; discovery refreshes preserve that publication hold and do not rewrite the held event even when global auto-publication is enabled. Provider/external identity and the candidate/event audit key make retries return the existing link. Once linked, replays do not overwrite event fields, so later manual edits are preserved; a later publish request for a still-private linked event returns a conflict instead of silently claiming success. After an administrator makes the event public through the normal event workflow, replaying with `publish=true` releases the provider hold.
