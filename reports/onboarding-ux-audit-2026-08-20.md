...
- Native validation additionally moved the global offline alert below the system status bar, localized it with the active Spanish/English preference, exposed it as a live accessibility alert, and reserved its lane on onboarding. The shared optional-field suffix now follows the auth locale as well.
...
- The locked mobile install restores the declared `expo-notifications` and `expo-haptics` runtime modules. Expo config evaluation and a clean static web export of all 50 routes pass. The current mobile TypeScript check passes with zero diagnostics.
...
- A current Android debug APK built successfully against SDK 36 and ran on the `Medium_Phone_API_36.1` x86_64 AVD. At 1.0 and 2.0 Android font scales, fresh-launch Spanish and English onboarding rendered without clipping or overlap. The 2.0 accessibility tree placed the offline alert at y=63–183, the onboarding panel at y=231–2303 inside the y=63–2337 scroll viewport, and both account actions within that viewport. The signup handoff remained scrollable through every field, five intent choices, terms/privacy links, separate marketing consent, submit action, and session controls; its English optional label was verified as `Last name (optional)`.
...
- Final native sign-off is now limited to a small iPhone, a modern iPhone, VoiceOver, and a hands-on TalkBack pass. Android visual, touch-target, localization, scroll-reachability, and accessibility-tree checks are complete. Initial AVD system/launcher watchdogs occurred while the host volume was full; after preserving the APK and removing reproducible Gradle intermediates, the installed TDF process completed both font-scale runs without an app crash.
