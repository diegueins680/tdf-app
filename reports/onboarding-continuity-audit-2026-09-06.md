...
The highest-priority acquisition actions also improve: a guest following an artist or contacting a directory profile is returned to that exact server-loaded target after authentication and receives an explicit confirmation. No follow or message is executed silently. Artist-follow and feature-access-request completion now require Party-bound domain evidence inside the signup window. Mobile restores interrupted intent, retains it after transient persistence failure, and uses it as the post-login fallback when no safe task-specific return URL exists.

The existing onboarding experiment remains paused. Its exposure state is still device-local, so it is not ready to reactivate. Event saving itself also remains device-local on mobile, but it is now isolated by authenticated Party and storage failures no longer report false success. The onboarding completion marker becomes account-durable only after the local save actually succeeds.
...
| Test runners | Available | Web/mobile Jest, TypeScript, ESLint, Node release tests, catalog audit, migration test, and Stack were available. The second-batch focused web/mobile suites, full mobile suite, type/build/lint, Node safeguards, backend compile/link, and focused Hspec executable ran. | Executed results are separated from configured, interrupted, or skipped checks. |
...
| Backend `/session/onboarding*` | Any authenticated Party | All clients | No row, eligible, completed, expired, invalid intent/value, missing/cross-Party/pre-signup/future evidence, duplicate completion, explicit exit | Haskell source/unit, OpenAPI, migration test | Implemented; current test target compiled/linked, focused onboarding passed 3/3, and access-request evidence passed 1/1; concurrency not load-tested |
...
| Mobile events/social/access request | Customer/Fan/requester | iOS/Android source | Action success, duplicate completion, API/storage failure, account switch, corrupt/legacy storage | Source/Jest | Completion marker durable; event save payload remains device-local but is Party-scoped and truthful |
...
| Mobile saved events leak across accounts and swallow persistence failures | Resolved in implementation; native runtime pending | Party-scoped keys/cache identities, quarantined legacy data, propagated errors, retry states, stale-account guards, and mocked storage/component tests. |
...
| **MEASURE-01** Claimed first value | Directly call completion with an allowlisted value. Expected for authoritative analytics: action independently exists. Baseline: endpoint deduplicated a client-observed success without verifying the domain record. | Server/API/UI source; **medium, high** measurement integrity. | Add per-domain evidence checks while preserving the completion handshake and exactly-once analytics. Medium, domain-specific. | `artist_followed` now requires a Party-bound follow and `access_requested` requires a Party-bound feature-access request created between signup and the completion request; missing, other-Party, pre-signup, and future evidence fail closed. Event-save and reaction evidence remain staged. **Partially implemented; focused SQLite tests compiled and passed locally.** |
| **MOB-STATE-02** Saved events | Baseline: save as account A, switch to B on the same device, or force AsyncStorage failure. Expected: strict account isolation and truthful failure. Actual: one global key leaked A's interests and swallowed writes while UI/analytics claimed success; saves also did not cross devices. | Mobile storage/auth/events source; **high privacy/data-integrity severity, high confidence**. | Immediate containment: Party-scoped key/cache/mutation identity, preserve ambiguous legacy/corrupt data, surface retry/error, suppress stale-account analytics. Follow-up: reuse and harden account-scoped `directory_favorite` for remote sync. | Same-device account isolation and truthful local persistence are **implemented and mocked/full-Jest verified**; cross-device convergence remains **deferred**. |
...
No uplift, completion rate, or conversion rate is claimed. The completion endpoint remains idempotent. Artist-follow and access-request completion are now backed by Party-bound domain records inside the eligibility window; event-save and reaction labels remain client-observed and are not presented as authoritative measurement.
...
| Mobile full Jest | 66 suites / 340 tests passed after intent retry/routing and saved-event containment changes | Full mobile JavaScript test collection ran with `REQUIRE_MOBILE_WORKSPACE=1`; not native device/backend integration. |
| Mobile saved-event focused Jest | 2 suites / 9 tests passed | Party isolation, legacy/corrupt preservation, storage failure propagation, and no false Event-detail success pass with mocks. |
...
- `src/lib/savedEvents.ts`, `app/eventDetail.tsx`, `app/userProfile.tsx`
- Focused auth, API, provider, gate, flags, intent, saved-event storage, and Event-detail persistence tests
...
1. **Complete server-observed first value.** Impact: saved-event and reaction completion can still be forged or missed; artist follow and access request are now checked. Dependency: per-domain evidence/history plus durable saved-event storage. Acceptance: durable evidence proves every accepted label with retry and no duplicate conversion.
2. **Cross-device saved events.** Impact: a “saved” mobile event does not follow the account, although same-device account isolation and truthful storage errors are now implemented. Dependency: harden/reuse the existing `directory_favorite` event path plus offline policy. Acceptance: two devices converge, explicit provenance-safe legacy import exists, canonical event visibility is validated, and server acknowledgement provides authoritative `event_saved` evidence.
